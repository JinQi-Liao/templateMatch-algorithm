...
//7.使用BPCj算法进行模板匹配，单分区的模板匹配算法
//7. The match Processing of BPCj
cv::Point BPCMatchProcessing(const Mat & srcImage, const Mat & templateImage, double NCCMAX, double ratio1)
{
	...
	for (int i = 0; i < subSqSumImage1.rows; i++)
	{
		for (int j = 0; j < subSqSumImage1.cols; j++)
		{
			...
			double boundTerm2 = (subSqSumImage2.at<double>(i, j) + partTSum) / 2;
			...
			{
				double residualTerm = NCCPartTerm(j, i, endRow, templateImage.rows, srcImage, templateImage);
...
int main()
{
	...
	for (int i = 0; i < nums; i++)
	{
		...
		Mat srcImage = imread("6-11.bmp", 0);
		Mat templateImage = imread("T6.bmp", 0);

		//Set initialThreshold and correlRation
		double NCCMAX = 0.9;
		double correlRatio = 0.5;
		int scale = 1;

		//Test matching time
		double start_time = static_cast<double>(getTickCount());

		Point bestpoint = twoStageMatchProcessing(srcImage, templateImage, NCCMAX, correlRatio, scale);
